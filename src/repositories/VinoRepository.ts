...
export class VinoRepository {
    ...
    async getAllVinos(): Promise<Vino[]> {
        ...
        const vinos: Vino[] = await Promise.all(vinosEntities.map(async (vinoEntity) => this.mapToVino(vinoEntity)));

        // console.log('vinos', vinos)
        return vinos;
    }

    private async mapToVino(vinoEntity: VinoEntity): Promise<Vino> {
        ...
        const maridajes = (await vinoEntity.maridajes).map(maridajeEntity =>
            new Maridaje(maridajeEntity.descripcion,
                maridajeEntity.nombre,),
        );
        ...
        const resenas = (await vinoEntity.resenas).map(resenaEntity =>
            new Resena(resenaEntity.comentario,
                resenaEntity.premium,
                resenaEntity.fechaResena,
                resenaEntity.puntaje)
        );
    ...
    private getPaisInstance(paisEntity: PaisEntity): Pais {
    ...
    private getProvinciaInstance(provinciaEntity: ProvinciaEntity): Provincia {
    ...
    private getRegionVitivinicolaInstance(regionEntity: RegionVitivinicolaEntity): RegionVitivinicola {
    ...
    private getBodegaInstance(bodegaEntity: BodegaEntity): Bodega {
    ...
    private getTipoUvaInstance(tipoUvaEntity: TipoUvaEntity): TipoUva {
